#!/usr/bin/env python3
"""Build orchestrator for C# Cross-platform Desktop UI Framework."""
...
def run(cmd: List[str], cwd: Optional[Path] = None, env: Optional[dict] = None, timeout: int = 600) -> Tuple[int, str]:
    ...
    print("+", " ".join(cmd))
    try:
        p = subprocess.run(cmd, cwd=str(cwd or ROOT), env=merged, capture_output=True, text=True, timeout=timeout)
...
def validate_versions(report: Report) -> Tuple[int, int]:
    ...
    try:
        ...
        return 0, 0
    props = read_props()
    expect_fe, expect_be = f"net{fe}.0", f"net{be}.0"
    fe_eval, be_eval = evaluate_tfm(ROOT / "frontend_csharp" / "frontend_csharp.csproj"), evaluate_tfm(ROOT / "backend_csharp" / "backend_csharp.csproj")
    ok = (
        props.get("FrontendTargetFramework") == expect_fe
        and props.get("BackendTargetFramework") == expect_be
        and props.get("FrontendDotnetVersion") == str(fe)
        and props.get("BackendDotnetVersion") == str(be)
        and props.get("BranchName") == branch
        and fe_eval == expect_fe
        and be_eval == expect_be
    )
...
def run_unit_tests(report: Report) -> None:
    ...
    projects = ["shared/tests/tests.csproj", "backend_csharp/tests/tests.csproj", "frontend_csharp/tests/tests.csproj"]
    ok, details = True, []
...
def run_tool(name: str, report: Report) -> None:
    ...
    out_dir = REPORTS / mapping.get(name, name)
    ...
    if name == "nuget-audit":
        ...
        report.set("NuGet-Audit", code == 0, "package audit written")
        return
    ...
    if name == "opentelemetry":
        ...
        live = False
        try:
            with urllib.request.urlopen("http://localhost:5081/health", timeout=2) as resp:
    ...
    if name == "roslyn":
        ...
        report.set("Roslyn", code == 0, "backend roslyn/compiler analysis")
        return
    ...
    if name == "semgrep":
        findings = []
        for folder in ["shared", "frontend_csharp", "backend_csharp", "desktop", "distribution"]:
            base = ROOT / folder
            if not base.exists():
                continue
        ...
        code, out = run(["semgrep", "--config", "quality/config/semgrep.yml", "--json", "-o", str(out_dir / "semgrep.json"),
                         "shared", "frontend_csharp", "backend_csharp", "desktop", "distribution"])
    ...
    if name == "jscpd":
        targets = ["shared", "frontend_csharp", "backend_csharp", "desktop", "distribution"]
    ...
    if name == "lizard":
        ...
        code, out = run([sys.executable, "-m", "lizard", "desktop", "distribution", "shared/src", "backend_csharp/src", "frontend_csharp/src"])
        (out_dir / "lizard.txt").write_text(out, encoding="utf-8")
        report.set("lizard", code == 0, "complexity analysis")
        return
    ...
    if name == "pydriller":
        ...
        script = (
            ...
            f"open(r'{out_dir / 'pydriller.json'}','w',encoding='utf-8').write(__import__('json').dumps({{'commits':len(commits)}},indent=2))\n"
            "print(len(commits))\n"
        )
    ...
    if name == "stryker":
        ...
        if stryker.exists():
            ...
            (out_dir / "stryker.txt").write_text("stryker tool unavailable; dry-run recorded", encoding="utf-8")
...
def start_platform(report: Report, fe: int, be: int) -> Optional[subprocess.Popen]:
    if not report.ok("Backend Build"):
        report.set("Desktop Startup", False, "backend build failed", blocked=True)
        return None
    env = os.environ.copy()
    env.update({
    ...
    for _ in range(60):
        if proc.poll() is not None:
            break
        try:
            with urllib.request.urlopen("http://localhost:5081/health", timeout=1) as resp:
                if resp.status == 200:
                    healthy = True
                    break
        except Exception:
            time.sleep(0.5)
    report.set("Desktop Startup", healthy, "http://localhost:5081/health")
...
def desktop_e2e(report: Report) -> None:
    if not report.ok("Desktop Startup"):
        for k in ["Desktop Initialization", "OPEN", "UPDATE", "CLOSE", "Sync/Replication", "TTL", "Eviction", "Statistics", "Frontend/Backend E2E"]:
            report.set(k, False, "startup failed", blocked=True)
        return

    code, health = http_json("GET", "http://localhost:5081/health")
    report.set("Desktop Initialization", code == 200 and health.get("desktop") == "available", json.dumps(health))

    code, opened = http_json("POST", "http://localhost:5081/desktop/windows", {"windowId": "window:1001", "title": "MainShell"})
    report.set("OPEN", opened.get("success") is True and "OPEN" in opened.get("message", ""), json.dumps(opened)[:300])

    code, updated = http_json("PUT", "http://localhost:5081/desktop/windows/window%3A1001", {"payload": "Visvantha"})
    code2, state = http_json("GET", "http://localhost:5081/desktop/windows/window%3A1001")
    update_ok = updated.get("success") is True and state.get("window", {}).get("payload", {}).get("content") == "Visvantha"
    report.set("UPDATE", update_ok, json.dumps(updated)[:300])

    code3, synced = http_json("GET", "http://localhost:5081/desktop/windows/window%3A1001?synced=true")
    report.set("Sync/Replication", synced.get("success") is True and synced.get("window", {}).get("payload", {}).get("content") == "Visvantha", json.dumps(synced)[:300])

    code, stats = http_json("GET", "http://localhost:5081/desktop/stats")
    stats_ok = all(k in stats for k in ["open_count", "update_count", "accepted_update_count", "active_windows"])
    report.set("Statistics", stats_ok, json.dumps(stats)[:300])

    code, _ = http_json("POST", "http://localhost:5081/desktop/windows", {"windowId": "window:ttl-e2e", "title": "TTL"})
    code, ttl_get = http_json("GET", "http://localhost:5081/desktop/windows/window%3Attl-e2e")
    report.set("TTL", ttl_get.get("success") is True, "session active before TTL; unit tests cover expiry")
    report.set("Eviction", report.ok("Unit Tests"), "covered by DesktopAppManagerTest.Eviction_RespectsCapacity")

    code, closed = http_json("DELETE", "http://localhost:5081/desktop/windows/window%3A1001")
    code2, after = http_json("GET", "http://localhost:5081/desktop/windows/window%3A1001")
    report.set("CLOSE", closed.get("success") is True and after.get("success") is False, json.dumps({"close": closed, "get": after})[:300])

    code, version = http_json("GET", "http://localhost:5081/version")
...
def print_final(report: Report) -> int:
    ...
    required = [
        ...
        "Desktop Startup", "Desktop Initialization", "OPEN", "UPDATE", "CLOSE",
        "Sync/Replication", "TTL", "Eviction", "Statistics", "Frontend/Backend E2E",
